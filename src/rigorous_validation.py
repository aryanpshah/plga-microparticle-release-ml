...
def rigorous_validation(raw_path: str, initial_path: str, output_dir: Optional[str] = None) -> None:
    ...
    if len(y_train_cls) == 0 or len(y_test_cls) == 0:
        logger.warning("Burst classifier split is empty after filtering; skipping classifier fit.")
        train_preds = np.array([], dtype=int)
        test_preds = np.array([], dtype=int)
    elif np.unique(y_train_cls).size < 2:
        logger.warning("Burst classifier training split has a single class; using constant predictor for validation.")
        train_preds = np.repeat(y_train_cls[0], len(y_train_cls))
        test_preds = np.repeat(y_train_cls[0], len(y_test_cls))
    else:
    ...
    train_acc = accuracy_score(y_train_cls, train_preds) if len(y_train_cls) else np.nan
    test_acc = accuracy_score(y_test_cls, test_preds) if len(y_test_cls) else np.nan
    ...
    if len(y_test_cls):
        ...
        logger.info("  -> Test Confusion Matrix: unavailable (empty test split)")
    if not np.isnan(test_acc) and test_acc < 0.99:
        logger.info("  [CONCLUSION] Leakage Confirmed. 100%% was an artifact of improper validation.")
    elif not np.isnan(test_acc):
